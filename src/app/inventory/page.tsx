  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-900">Inventory & Godown Stocks</h1>
          <p className="text-xs text-slate-500">Track multi-warehouse inventory, FEFO batches, manufacturing & expiry dates</p>
        </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {batches.map((b) => (
                  <tr key={b.id} className="hover:bg-slate-50/80">
                    ...
                    <td className="p-3.5 text-slate-600">{b.warehouse?.name}</td>
              ...
              <tbody className="divide-y divide-slate-100">
                {movements.map((m) => (
                  <tr key={m.id} className="hover:bg-slate-50/80">
                    ...
                    <td className="p-3.5 text-slate-600">{m.warehouse?.name}</td>
                    <td className="p-3.5">
                      <span className={clsx(
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200 text-xs">
            ...
            <form onSubmit={handleStockAdjustment} className="space-y-3.5">
              <div>
                <label className="block font-semibold text-slate-700 mb-1">Warehouse Godown *</label>
                <select
                  required
      ...
      {showBatchModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200 text-xs">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-4">
              <h2 className="text-base font-bold text-slate-900">Receive New Product Batch</h2>
            ...
            <form onSubmit={handleCreateBatch} className="space-y-3.5">
              ...
              <div>
                <label className="block font-semibold text-slate-700 mb-1">Batch Number / Lot # *</label>
                <input
                  type="text"
                  required
                  value={batchForm.batchNumber}
                  onChange={(e) => setBatchForm({ ...batchForm, batchNumber: e.target.value })}
                  placeholder="e.g. BAT-2026-AUG01"
