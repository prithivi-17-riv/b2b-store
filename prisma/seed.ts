...
async function main() {
  ...
  // 10. Additional Demo Orders & Invoices for Showcase
  const abcCust = await prisma.customer.findFirst({ where: { customerCode: 'CUST-002' } });
  const kumarCust = await prisma.customer.findFirst({ where: { customerCode: 'CUST-003' } });
  const xyzCust = await prisma.customer.findFirst({ where: { customerCode: 'CUST-004' } });
  const muruganCust = await prisma.customer.findFirst({ where: { customerCode: 'CUST-005' } });

  const sonaProduct = await prisma.product.findFirst({ where: { sku: 'RICE-SON-02' } });
  const attaProduct = await prisma.product.findFirst({ where: { sku: 'FLR-ATTA-01' } });
  const teaProduct = await prisma.product.findFirst({ where: { sku: 'FMC-TEA-01' } });
  const chilliProduct = await prisma.product.findFirst({ where: { sku: 'SPC-CHL-01' } });
  const saltProduct = await prisma.product.findFirst({ where: { sku: 'SPC-SLT-02' } });
  const surfProduct = await prisma.product.findFirst({ where: { sku: 'CLN-SRF-01' } });
  const moongProduct = await prisma.product.findFirst({ where: { sku: 'PUL-MOON-03' } });

  // Order 2: ABC Supermarket - Delivered & Paid
  if (abcCust && sonaProduct && attaProduct && surfProduct) {
    const order2 = await prisma.order.create({
      data: {
        orderNumber: 'ORD-2026-5002',
        customerId: abcCust.id,
        salesEmployeeId: sales1.id,
        priceCategoryId: pcWholesale.id,
        warehouseId: whCentral.id,
        status: 'DELIVERED',
        ...
        roundOff: 0,
        grandTotal: 104775,
        notes: 'Bi-weekly wholesale grocery replenishment',
        items: {
          create: [
            {
              productId: sonaProduct.id,
              productName: sonaProduct.name,
              hsnCode: sonaProduct.hsnCode,
              uom: sonaProduct.uom,
              quantity: 40,
              unitRate: 1200,
              discountPercent: 0,
            ...
            {
              productId: attaProduct.id,
              productName: attaProduct.name,
              hsnCode: attaProduct.hsnCode,
              uom: attaProduct.uom,
              quantity: 30,
              unitRate: 750,
              discountPercent: 0,
            ...
            {
              productId: surfProduct.id,
              productName: surfProduct.name,
              hsnCode: surfProduct.hsnCode,
              uom: surfProduct.uom,
              quantity: 15,
              unitRate: 1850,
              discountPercent: 0,
    ...
    const invoice2 = await prisma.invoice.create({
      data: {
        ...
        placeOfSupplyStateCode: '33',
        isInterstate: false,
        billingAddress: abcCust.billingAddress,
        shippingAddress: abcCust.deliveryAddress,
        customerGstin: abcCust.gstin,
        taxableAmount: 98250,
        cgstAmount: 3262.5,
        sgstAmount: 3262.5,
        igstAmount: 0,
        ...
        roundOff: 0,
        grandTotal: 104775,
        amountInWords: 'One Lakh Four Thousand Seven Hundred and Seventy Five Rupees Only',
        paidAmount: 104775,
        balanceAmount: 0,
        paymentStatus: 'PAID',
        items: {
    ...
    await prisma.payment.create({
      data: {
        paymentNumber: 'PAY-2026-0802',
        customerId: abcCust.id,
        invoiceId: invoice2.id,
        amount: 104775,
        paymentMethod: 'NEFT',
        referenceNumber: 'NEFT/HDFC/20260826/889211',
        paymentDate: new Date('2026-08-26'),
        receivedByUserId: sales1.id,
        notes: 'Full invoice settlement via NEFT bank transfer',
      },
    });
  }

  // Order 3: Kumar Traders - Confirmed
  if (kumarCust && toorProduct && sugarProduct) {
    await prisma.order.create({
      data: {
        orderNumber: 'ORD-2026-5003',
        customerId: kumarCust.id,
        salesEmployeeId: sales2.id,
        priceCategoryId: pcWholesale.id,
        warehouseId: whCentral.id,
        status: 'CONFIRMED',
        orderDate: new Date('2026-08-28'),
        itemCount: 2,
        totalQuantity: 150,
        subtotal: 11900,
        discountAmount: 0,
        ...
        roundOff: 0,
        grandTotal: 12495,
        notes: 'Standard wholesale stock order',
        items: {
          create: [
            {
              productId: toorProduct.id,
  ...
  // Order 4: XYZ Mega Mart (Bangalore) - Interstate IGST (Karnataka)
  if (xyzCust && teaProduct && chilliProduct && saltProduct) {
    const order4 = await prisma.order.create({
      data: {
        ...
        warehouseId: whCentral.id,
        status: 'DELIVERED',
        orderDate: new Date('2026-08-30'),
        itemCount: 3,
        totalQuantity: 120,
        subtotal: 187400,
        discountAmount: 0,
        taxableAmount: 187400,
        cgstAmount: 0,
        sgstAmount: 0,
        igstAmount: 9370,
        cessAmount: 0,
        roundOff: 0,
        grandTotal: 196770,
        notes: 'Interstate Bulk Consignment - IGST 5% applicable',
        items: {
          create: [
            {
              productId: teaProduct.id,
              productName: teaProduct.name,
              hsnCode: teaProduct.hsnCode,
              uom: teaProduct.uom,
              quantity: 30,
              unitRate: 2580,
              discountPercent: 0,
            ...
            },
            {
              ...
              productName: chilliProduct.name,
              hsnCode: chilliProduct.hsnCode,
              uom: chilliProduct.uom,
              quantity: 40,
              unitRate: 2150,
              discountPercent: 0,
              discountAmount: 0,
            ...
            },
            {
              ...
              productName: saltProduct.name,
              hsnCode: saltProduct.hsnCode,
              uom: saltProduct.uom,
              quantity: 50,
              unitRate: 480,
              discountPercent: 0,
    ...
    await prisma.invoice.create({
      data: {
        ...
        isInterstate: true,
        billingAddress: xyzCust.billingAddress,
        shippingAddress: xyzCust.deliveryAddress,
        customerGstin: xyzCust.gstin,
        taxableAmount: 187400,
        cgstAmount: 0,
        sgstAmount: 0,
        igstAmount: 9370,
        ...
        roundOff: 0,
        grandTotal: 196770,
        amountInWords: 'One Lakh Ninety Six Thousand Seven Hundred and Seventy Rupees Only',
        paidAmount: 196770,
        balanceAmount: 0,
        paymentStatus: 'PAID',
        items: {
  ...
  // Order 5: Murugan Kirana & Provisions - Pending Order
  if (muruganCust && moongProduct && ponniProduct) {
    await prisma.order.create({
      data: {
        orderNumber: 'ORD-2026-5005',
        customerId: muruganCust.id,
        salesEmployeeId: sales2.id,
        priceCategoryId: pcRetail.id,
        warehouseId: whCentral.id,
        status: 'PENDING',
        ...
        roundOff: 0.5,
        grandTotal: 18638,
        notes: 'Weekly kirana store restocking order',
        items: {
          create: [
            {
              productId: moongProduct.id,
              productName: moongProduct.name,
              hsnCode: moongProduct.hsnCode,
              uom: moongProduct.uom,
              quantity: 25,
              unitRate: 130,
              discountPercent: 0,
              discountAmount: 0,
